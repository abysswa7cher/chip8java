...
public class Chip8 {
    ...
    public String dumpState(Boolean withRam) {
        int[] registers = cpu.getRegisters(); int Iregister = cpu.getIregister(); int PCregister = cpu.getPCregister();
        int SPregister = cpu.getSPregister(); int[] stack = getStack();
        ...
        StringBuilder sb = new StringBuilder(); if (withRam) sb.append(printRAM())
                .append("\n\n======================================================================\n\n");

        for (int i = 0; i < registers.length; i++) {
            String ihex = Integer.toHexString(i).toUpperCase(); String regval = Integer.toHexString(registers[i]);
            if (i % 4 == 0) sb.append("\n"); if (registers[i] == 0) sb.append(ihex).append(": 00");
            else if (registers[i] < 16) sb.append(ihex).append(": 0").append(regval);
            else sb.append(ihex).append(": ").append(regval); if (i < registers.length - 1) sb.append("   ");
        }

        sb.append("\n").append("VI: ").append(IregisterS).append(" [").append(ramAtI).append("]").append("\n")
                .append("PC: ").append(PCregisterS).append(" [").append(ramAtPC).append(" ").append(ramAtPC2)
                .append("]").append("\n").append("SP: ").append(SPregisterS).append(" [").append(ramAtSP).append("]")
                .append("\n").append("Stack: ").append(Arrays.toString(stack)).append("\n\n"); return sb.toString();
    }

    public String printRAM() {
        StringBuilder sb = new StringBuilder(); int printCount = 0; for (int i = 0; i < ram.length; i++) {
            if (ram[i] > 0 || (i >= 512 && i <= 512 + cpu.programSize)) {
                if (printCount % 10 == 0) sb.append("\n"); sb.append(Integer.toHexString(i).toUpperCase()).append(": ");
                if (ram[i] < 16) sb.append("0").append(Integer.toHexString(ram[i]).toUpperCase());
                else sb.append(Integer.toHexString(ram[i]).toUpperCase()); if (i < ram.length - 1) sb.append(' ');
                printCount++;
            }
        } return sb.toString();
    }
